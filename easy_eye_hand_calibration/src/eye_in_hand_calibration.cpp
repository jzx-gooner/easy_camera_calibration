...
int num_of_all_images =15;
// 行列方向内角点数量,每行有几个圆
cv::Size board_size = cv::Size(7, 7);
// 标定板棋盘格实际尺寸(单位要与pose.txt中机器人位置的单位一致) mm
cv::Size2f square_size = cv::Size2f(15, 15);
...
int handEye_calib(Eigen::Matrix4d &gHc, std::string path)
{
	...
	for (int i = 0; i<images.size(); i++)
	{
		...
		if (0 == cv::findCirclesGrid(image, board_size, image_points_buff))
		{
	...
	for (int i = 0; i<num_of_images; i++)
	{
		...
		Eigen::Quaterniond rot_temp(v[0],v[1],v[2],v[3]);
	...
	Eigen::Quaterniond q((Matrix3d)gHc.topLeftCorner(3, 3));
